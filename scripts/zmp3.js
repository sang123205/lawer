...
async function reply({ event, api, reply, args, global }) {
  ...
  try {
    ...
    fs.writeFileSync(__dirname + `/cache/${reply.link[event.body - 1]}.mp3`, Buffer.from(getms, "utf-8"));
    if (fs.statSync(__dirname + `/cache/${reply.link[event.body - 1]}.mp3`).size > 26214400) return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => fs.unlinkSync(__dirname + `/cache/${reply.link[event.body - 1]}.mp3`), event.messageID);
    else api.sendMessage({ body: `Link Tải: ${shortLink}`, attachment: fs.createReadStream(__dirname + `/cache/${reply.link[event.body - 1]}.mp3`) }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/${reply.link[event.body - 1]}.mp3`), event.messageID);
  }
  catch {
...
async function out({ event, api, global, Config, logger, args }) {
  const axios = require('axios');
  ...
  try {
    ...
    return api.sendMessage(`🔊 Có ${link.length} kết quả trùng với từ khoá tìm kiếm của bạn:\n\n${msg}\nHãy reply(phản hồi) chọn một trong những tìm kiếm trên`, event.threadID, (error, info) => global.reply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, link }), event.messageID);
  }
  catch {
