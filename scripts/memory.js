...
module.exports = {
  config: {
    ...
    ver: '1.0.0',
    role: 0,
    author: ['NTK_CV Lawer Team'],
    description: 'Trò chơi thử thách trí nhớ của bạn.',
    location: __filename,
    timestamps: 5
  }
};
...
module.exports.onEvent = async function({ api, event, args }) {
...
module.exports.onMessage = async function({ api, event, args }) {
  ...
  const { registerFont, loadImage, createCanvas, Canvas } = require("canvas");
  ...
  let getfont = (await axios.get(`https://github.com/manhkhac/mirai-1.2.8/raw/data/font/Montserrat-Bold.ttf`, { responseType: "arraybuffer" })).data;
  fs.writeFileSync(__dirname + "/cache/memory.ttf", Buffer.from(getfont, "utf-8"));
  }
	var imgrd = "https://raw.githubusercontent.com/manhkhac/mirai-1.2.8/data/img/memory.jpg";
 ...
 fs.writeFileSync(pathImg, imageBuffer);
  ...
  return api.sendMessage(
    { body: "You have 10 seconds to memorize the content below, then retype it!!", attachment: fs.createReadStream(pathImg) }, threadID, async (e, info) => {
